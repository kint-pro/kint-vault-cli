...
// can handle interspersed flags (like "delete KEY -y") the way Python's
// argparse does. It recognizes known flag names from the FlagSet.
func reorderArgs(fs *flag.FlagSet, args []string) []string {
	...
	for i := 0; i < len(args); i++ {
		...
		if name != "" && known[name] {
			...
			// If this flag takes a value and it's not in "flag=value" form, consume next arg
...
func main() {
	...
	// Handle --version and --help before subcommand dispatch
	...
	case "run":
		// Special: everything after -- is the command to run, don't reorder
		fs := flag.NewFlagSet("run", flag.ExitOnError)
		env := fs.String("env", "", "Override environment")
		// Find -- separator, parse only flags before it
