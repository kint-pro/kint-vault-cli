...
func CmdSet(envOverride string, pairs []string) {
	...
	result, err := backend.DecryptForUpdate(cfg)
	if err != nil {
		fatal(err.Error())
	}
	...
	if result != nil {
		if err := backend.EncryptContentWithKey(cfg, envfile.Format(secrets), result.DataKey, result.Cipher); err != nil {
			fatal(err.Error())
		}
	} else {
		if err := backend.EncryptContent(cfg, envfile.Format(secrets)); err != nil {
			fatal(err.Error())
		}
	}
}
...
func CmdGet(envOverride, key string) {
	...
	content, err := backend.Decrypt(cfg, "")
	if err != nil {
		fatal(err.Error())
	}
...
func CmdList(envOverride string, asJSON, all bool) {
	...
	if all {
		...
		results := runParallel(len(encFiles), func(i int) (string, interface{}, error) {
			...
			content, err := backend.Decrypt(cfg, dir)
			if err != nil {
				return "", nil, err
			}
